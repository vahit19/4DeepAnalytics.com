...
# Coronalytics, a project for Analytics of COVID-19 Rules For Wearing Face Masks and Social Distances.
# AI using Deep Learning Technology For Identifying a Person of Interest (POI) in Real World
# List of Istanbul city Live Cameras - Streaming URLs
# Vahit FERYAD - vahit.feryad@4deepanalytics.com, vahit.feryat@gmail.com
# this project will be custom trained for specific use-cases of real world Analytics
...
import Istanbul_Live_Cams

url = Istanbul_Live_Cams.selected()
...
class SocialDistanceAnalysis:
	...
	def generatePieChart(self, safe, risk):
		...
		ax.pie([safe, risk], labels = ['Guvenli', 'Risk'])
		ax.legend()
		plt.savefig("pie.png", transparent=True, dpi=100)
		plt.close()
	...
	def populateDashboard(self, emptyDash, safe, risk):
		...
		pieoffsets = self.overlayImage(emptyDash, plt, (10,150))
		...
		cv2.putText(emptyDash, str(totalPeople), totalOffsets[1], cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
		cv2.putText(emptyDash, "Total", (totalOffsets[1][0]+50,totalOffsets[1][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)

		emptyDash = self.overlayImage(emptyDash, self.safeIcon, safeOffsets[0])
		cv2.putText(emptyDash, str(safe), safeOffsets[1], cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
		cv2.putText(emptyDash, "Guvenli Kisi Sayisi", (safeOffsets[1][0]+50,safeOffsets[1][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0),2)

		emptyDash = self.overlayImage(emptyDash, self.riskIcon, riskOffsets[0])
		cv2.putText(emptyDash, str(risk), riskOffsets[1], cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
		cv2.putText(emptyDash, "Riskli Kisi Sayisi", (riskOffsets[1][0]+50,riskOffsets[1][1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0),2)
	...
	def SDASetup(self, yolopath, assetpath, videoShape):
		if self.setup:
			...
			cv2.putText(self.blankDashboard, "Covid-19 Sosyal Mesafe Analizi", (20,30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
			cv2.putText(self.blankDashboard, "Istiklal Caddesi - Istanbul", (20,60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
			cv2.putText(self.blankDashboard, "Live Analytics Dashboard", (20,90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
			cv2.putText(self.blankDashboard, "Vahit FERYAD - PhD", (20,120), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)

			self.safeIcon = cv2.imread(assetpath + 'safeIcon.png')
			self.safeIcon = imutils.resize(self.safeIcon, width=int(50))

			self.riskIcon = cv2.imread(assetpath + 'riskIcon.png')
			self.riskIcon = imutils.resize(self.riskIcon, width=int(50))

			self.totalIcon = cv2.imread(assetpath + 'totalIcon.png')
			self.totalIcon = imutils.resize(self.totalIcon, width=int(50))
...
while(True):
	...
	currentImg = cv2.resize(currentImg, (1400, 920))
